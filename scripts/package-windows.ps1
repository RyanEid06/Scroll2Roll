...
$stage = [System.IO.Path]::GetFullPath((Join-Path $packageRoot 'Scroll2Roll-0.1.0-windows-x64'))
...
$archive = Join-Path $packageRoot 'Scroll2Roll-0.1.0-windows-x64.zip'
if (Test-Path -LiteralPath $archive) { Remove-Item -LiteralPath $archive -Force }
Compress-Archive -Path (Join-Path $stage '*') -DestinationPath $archive -CompressionLevel Optimal
$archiveHash = (Get-FileHash -Algorithm SHA256 -LiteralPath $archive).Hash.ToLowerInvariant()
Set-Content -LiteralPath (Join-Path $packageRoot 'Scroll2Roll-0.1.0-windows-x64.zip.sha256') -Value "$archiveHash  Scroll2Roll-0.1.0-windows-x64.zip" -Encoding ascii

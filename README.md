...
The version-0.1.0 master-plan baseline remains preserved, and version 0.2.0 passed local acceptance with six complete games. All five owner-approved 0.3.0 games are complete with exact integer math, deterministic settlement, full keyboard/mouse flows, privacy-safe projections, and safe save-v1/save-v2/save-v3 migration to save-v4. The owner later rejected the native 0.3.0 visual baseline and mandated the complete shell-plus-eleven-interior overhaul in `UI_OVERHAUL.md`. That replacement now passes 48/48 tests in Debug and Release. It includes original ImageGen covers, licensed bundled Manrope Medium, purpose-designed dark/light themes, reduced motion, tested resource ownership, branded startup, a responsive eleven-card lobby, persistent state-safe Back, exact game guides, and eleven responsive game interiors with committed-result animation. The aligned static site uses verified native captures, and the package workflow includes and verifies the reviewed fonts, artwork, notices, recursive checksums, and relocated launch. Exact evidence is recorded in `docs/VALIDATION.md`; visual approval remains the owner's decision. The untouched original Blackjack draft remains under `legacy/Blackjack-v1`.
